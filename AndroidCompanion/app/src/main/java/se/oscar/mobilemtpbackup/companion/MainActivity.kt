...
class MainActivity : AppCompatActivity() {
    ...
    private val pairingCode: String = random.nextInt(1_000_000).toString().padStart(6, '0')
    ...
    private fun refreshStatus() = thread(name = "wifi-backup-status") {
        ...
        runOnUiThread { status.text = "Mobile MTP Backup Companion 0.7\n\nWi-Fi-tjänst aktiv\nIP-adress: $ip\nPort: 8765\nParkod: $pairingCode\nMediaåtkomst: $permission\nBilder/video synliga: $countText\n\nAnge IP-adressen och parkoden ovan i Windows-programmet.\nParkoden skickas inte längre i klartext: varje anslutning använder en ny nonce + HMAC-SHA256.\nLåt appen vara öppen under testet." }
    }
        ...
        try {
            ...
            while (!Thread.currentThread().isInterrupted) {
                ...
                try {
                    ...
                    when {
                        command=="HELLO" -> out.write("MOBILE_MTP_BACKUP_COMPANION/0.7\n".toByteArray())
                        ...
                        command.startsWith("GET ") -> { val id=command.removePrefix("GET ").toLongOrNull();val item=id?.let{queryMedia().firstOrNull{x->x.id==it}};if(item==null)out.write("ERROR not-found\n".toByteArray())else{val uri=android.content.ContentUris.withAppendedId(MediaStore.Files.getContentUri("external"),item.id);contentResolver.openInputStream(uri)?.use{input->out.write("DATA ${item.size}\n".toByteArray());input.copyTo(out)}?:out.write("ERROR open-failed\n".toByteArray())} }
