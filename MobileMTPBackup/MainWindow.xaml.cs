...
public partial class MainWindow : Window
{
    ...
    private void SystemTest_Click(object sender,RoutedEventArgs e){RefreshDevices();var d=SelectedDevice;if(d is null){MessageBox.Show("Ingen MTP-telefon hittades. Kontrollera att Enhetshanteraren visar MTP USB Device och klicka sedan Uppdatera enheter.","SYSTEMTEST");return;}try{var entries=_mtp.GetRootEntries(d);MessageBox.Show($"MTP OK: {d.Name}\nRoot innehåller {entries.Count} objekt.\n{_mtp.GetDiagnosticSummary()}","SYSTEMTEST");}catch(Exception ex){MessageBox.Show(ex.GetBaseException().Message,"SYSTEMTEST FEL");}}
    private void BackupFolder_Click(object sender,RoutedEventArgs e)=>MessageBox.Show("Hel-mapp backup aktiveras efter MTP-testet.");
    private void AutoBackup_Click(object sender,RoutedEventArgs e)=>MessageBox.Show("Auto-backup aktiveras efter MTP-testet.");
    private void Preview_Click(object sender,RoutedEventArgs e)=>MessageBox.Show("Förhandsgranskning aktiveras efter MTP-testet.");
