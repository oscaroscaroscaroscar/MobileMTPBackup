...
public partial class MainWindow
{
    private async void TestWifi_Click(object sender, RoutedEventArgs e)
    {
        ...
        string pairing=WifiPairCodeTextBox.Text.Trim();
        if(pairing.Length!=6||pairing.Any(c=>!char.IsDigit(c))){MessageBox.Show("Ange den 6-siffriga parkoden som visas i Android Companion.","WI-FI TEST");return;}
        WifiStatusText.Text="Testar Wi-Fi-anslutning och parkod...";
        try
        {
            using var client=new TcpClient();using var cts=new CancellationTokenSource(TimeSpan.FromSeconds(5));
            await client.ConnectAsync(host,port,cts.Token);
            using var stream=client.GetStream();byte[] hello=Encoding.UTF8.GetBytes($"AUTH {pairing} HELLO\n");await stream.WriteAsync(hello,cts.Token);await stream.FlushAsync(cts.Token);
            ...
            WifiStatusText.Text=$"Wi-Fi OK: {host}:{port} – Companion {reply.Split('/').Last()}, parkod godkänd.";Log($"WI-FI HANDSHAKE OK: {reply} från {host}:{port}; parkod godkänd.");
        }
